        ...
        <div className="fixed inset-x-3 bottom-3 sm:inset-x-auto sm:bottom-auto sm:absolute sm:right-0 sm:top-full sm:mt-2 w-[calc(100vw-1.5rem)] sm:w-[420px] max-h-[82vh] sm:max-h-[80vh] overflow-y-auto custom-scrollbar bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl sm:rounded-2xl shadow-2xl z-50 p-4 sm:p-4 animate-slide-up sm:animate-slide-down space-y-4 font-sans text-slate-800 dark:text-slate-100">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              <button
                onClick={() => setFilterAssigneeId(filterAssigneeId === 'UNASSIGNED' ? '' : 'UNASSIGNED')}
                className={`px-2.5 py-1 rounded-lg text-xs font-semibold flex items-center gap-1.5 border transition-all ${
                  filterAssigneeId === 'UNASSIGNED'
                    ? 'bg-amber-600 text-white border-amber-500 shadow-sm'
                    : 'bg-slate-100 dark:bg-slate-950/60 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 hover:text-amber-600 dark:hover:text-amber-300'
                }`}
              >
                <UserX className="w-3 h-3 text-amber-500 dark:text-amber-400" />
                <span>Sem responsável</span>
                {filterAssigneeId === 'UNASSIGNED' && <Check className="w-3 h-3 ml-0.5" />}
              </button>
